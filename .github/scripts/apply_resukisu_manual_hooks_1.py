...
def patch_defconfig(defconfig):
    ...
    remove_prefixes = [
        ...
        "CONFIG_KALLSYMS=",
        # 🔧 CHANGE: do NOT mass-strip KALLSYMS_ALL; keep whatever the
        # device defconfig had. We only force it if the kernel really
        # needs it (decided by env override).
    ...
    enable_kallsyms_all = os.environ.get("RESUKISU_KALLSYMS_ALL", "0") == "1"
    ...
    lines += [
        ...
        "CONFIG_KALLSYMS=y",
        # 🔧 CHANGE: gate KALLSYMS_ALL behind env var (was forced =y).
...
extern void ksu_handle_fstat64_ret(unsigned long *fd,
    ...
    data = read(path)
    if "SYSCALL_DEFINE4(fstatat64" in data and \
       "ksu_handle_stat(&dfd, &filename, &flag);" not in data:
        f64 = re.compile(
